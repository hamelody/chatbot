...
from streamlit_cookies_manager import EncryptedCookieManager # 수정된 임포트 (이전 답변에서 확인됨)
import traceback
import base64

st.set_page_config(
    page_title="유앤생명과학 업무 가이드 봇",
    layout="centered", # 이전 요청에 따라 centered로 유지
...
<style>
    ...
    .user-bubble { background-color: #90EE90; color: black; border-bottom-right-radius: 5px; } /* 사용자 말풍선 색상 (연초록) */
...
if openai_client: # OpenAI 클라이언트가 성공적으로 초기화된 경우에만 시도
    try:
...
def load_data_from_blob(blob_name, _container_client, data_description="데이터", default_value=None):
    if not _container_client:
        print(f"ERROR: Blob Container client is None for load_data_from_blob ('{data_description}').")
        return default_value if default_value is not None else {} # JSON의 경우 빈 딕셔너리, 리스트는 호출부에서 처리
    ...
    try:
        ...
        if blob_client_instance.exists():
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if os.path.getsize(local_temp_path) > 0: # 파일이 비어있지 않은 경우에만 load 시도
        ...
        st.error(f"'{data_description}' 파일({blob_name})이 손상되었거나 올바른 JSON 형식이 아닙니다.")
        return default_value if default_value is not None else {}
    except Exception as e:
        print(f"ERROR loading '{data_description}' from Blob '{blob_name}': {e}\n{traceback.format_exc()}")
        st.error(f"'{data_description}' 로드 중 오류 발생: {e}")
        return default_value if default_value is not None else {}
...
def save_data_to_blob(data_to_save, blob_name, _container_client, data_description="데이터"):
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            blob_client_instance = _container_client.get_blob_client(blob_name)
            with open(local_temp_path, "rb") as data_stream: # 바이너리 모드로 읽기
...
def save_binary_data_to_blob(local_file_path, blob_name, _container_client, data_description="바이너리 데이터"):
    if not _container_client:
        ...
        return False
    print(f"Attempting to save binary '{data_description}' to Blob: '{blob_name}'")
    try:
        blob_client_instance = _container_client.get_blob_client(blob_name)
...
if container_client:
    ...
    if not USERS or "admin" not in USERS : # 로드 실패 또는 admin 계정 부재 시
        ...
        if not save_data_to_blob(USERS, USERS_BLOB_NAME, container_client, "사용자 정보"):
             st.warning("기본 관리자 정보를 Blob에 저장하는데 실패했습니다.")
else:
    st.error("Azure Blob Storage 연결 실패로 사용자 정보를 초기화할 수 없습니다. 앱이 정상 동작하지 않을 수 있습니다.")
...
try:
    ...
    if not cookie_secret_key:
        ...
        cookies = EncryptedCookieManager(
            prefix="gmp_chatbot_final_auth/", # Prefix 변경 시 기존 쿠키와 호환 안됨
        ...
        if cookies.ready():
            ...
            print("CookieManager not ready on initial setup.")
except Exception as e:
...
if "authenticated" not in st.session_state:
    print("Initializing session state: 'authenticated', 'user', and 'messages'")
    st.session_state["authenticated"] = False
    st.session_state["user"] = {}
    st.session_state["messages"] = [] # 세션 시작 시 메시지 초기화
    if cookie_manager_ready:
        auth_cookie_val = cookies.get("authenticated")
        print(f"Cookie 'authenticated' value: {auth_cookie_val}")
            ...
            if (time.time() - login_time) < SESSION_TIMEOUT:
                ...
                try:
                    ...
                    if user_data_from_cookie:
                        ...
                        # st.session_state["messages"] = [] #  <--- 여기서 초기화하면 새로고침 시 항상 비워짐, 로그인/아웃 시점으로 이동
                        print(f"User '{user_data_from_cookie.get('name')}' authenticated from cookie.")
                    else:
                        print("User data in cookie is empty. Clearing auth state.")
                        st.session_state["authenticated"] = False
                ...
                print("Session timeout. Clearing auth state from cookie.")
                st.session_state["authenticated"] = False
                if cookies.ready(): cookies["authenticated"] = "false"; cookies["user"] = ""; cookies["login_time"] = ""; cookies.save()
    else:
        print("CookieManager not ready, cannot restore session from cookie.")
# --- 로그인 UI 및 로직 ---
if not st.session_state.get("authenticated", False):
    ...
    with st.form("auth_form_final", clear_on_submit=False): # form key 변경
        mode = st.radio("선택", ["로그인", "회원가입"], key="auth_mode_final")
        uid = st.text_input("ID", key="auth_uid_final")
        pwd = st.text_input("비밀번호", type="password", key="auth_pwd_final")
        name, dept = "", ""
        if mode == "회원가입":
            name = st.text_input("이름", key="auth_name_final")
            dept = st.text_input("부서", key="auth_dept_final")
            ...
            if mode == "로그인":
                ...
                if uid in USERS: st.error("이미 존재하는 ID입니다.")
                    ...
                    if not save_data_to_blob(USERS, USERS_BLOB_NAME, container_client, "사용자 정보"):
                        ...
                        # 실패 시 USERS 딕셔너리 롤백 (필요 시)
                        # current_users_copy = USERS.copy()
                        # current_users_copy.pop(uid, None) # 추가 시도했던 사용자 제거
                        # USERS = current_users_copy
                    else:
...
# --- 헤더 (로고, 버전, 로그아웃 버튼) ---
top_cols_main = st.columns([0.7, 0.3]) # 키 이름 변경 가능성 고려
    ...
    if os.path.exists(COMPANY_LOGO_PATH_REPO):
        ...
        if logo_b64:
            ...
            <div class="logo-container">
                ...
                <span class="version-text">ver 1.9 (Chat Clear Fix)</span>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div class="logo-container"><span class="version-text" style="font-weight:bold;">유앤생명과학</span> <span class="version-text" style="margin-left:10px;">ver 1.9 (Chat Clear Fix)</span></div>""", unsafe_allow_html=True)
    else:
...
with top_cols_main[1]:
    ...
    if st.button("로그아웃", key="logout_button_final"): # 키 이름 변경 가능성 고려
        st.session_state["authenticated"] = False
        st.session_state["user"] = {}
...
def load_vector_db_from_blob_cached(_container_client):
    if not _container_client:
        ...
        return faiss.IndexFlatL2(1536), [] # 빈 기본값 반환
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if index_blob_client.exists():
                ...
                # 파일 크기 체크 추가 (0바이트 파일 read_index 시 오류 방지)
                ...
                print(f"WARNING: '{INDEX_BLOB_NAME}' not found in Blob Storage.")

            metadata_blob_client = _container_client.get_blob_client(METADATA_BLOB_NAME)
            if metadata_blob_client.exists():
                with open(local_metadata_path, "wb") as download_file: # 바이너리로 쓰고
                    download_file.write(metadata_blob_client.download_blob().readall())
                if os.path.getsize(local_metadata_path) > 0: # 파일이 비어있지 않은 경우
                    with open(local_metadata_path, "r", encoding="utf-8") as f: # 텍스트로 읽기
                        meta = json.load(f)
...
index, metadata = faiss.IndexFlatL2(1536), [] # 초기화
if container_client:
    index, metadata = load_vector_db_from_blob_cached(container_client)
else:
    st.error("Azure Blob Storage 연결 실패로 벡터 DB를 로드할 수 없습니다. 일부 기능이 제한될 수 있습니다.")
...
def chunk_text_into_pieces(text_to_chunk, chunk_size=500):
    ...
    for line in text_to_chunk.split("\n"):
        ...
        if not stripped_line and not current_buffer.strip(): continue # 빈 줄 무시 (버퍼도 비었을 때)
...
def get_text_embedding(text_to_embed):
    if not openai_client or not EMBEDDING_MODEL:
        st.error("OpenAI 클라이언트 또는 임베딩 모델이 준비되지 않았습니다. 임베딩을 생성할 수 없습니다.")
...
def search_similar_chunks(query_text, k_results=5):
    ...
    try:
        # k_results 값을 index.ntotal 보다 작거나 같게 조정
        actual_k = min(k_results, index.ntotal)
        if actual_k == 0 : return [] # 검색할 아이템이 없음
...
def add_document_to_vector_db_and_blob(uploaded_file_obj, text_content, text_chunks, _container_client):
    ...
    for chunk_idx, chunk in enumerate(text_chunks): # 디버깅용 인덱스
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if index.ntotal > 0 : # 빈 인덱스는 저장하지 않거나, 빈 파일을 저장할 수 있음
            ...
            else: # 인덱스가 비어있는 경우, Blob에 빈 파일을 올리거나 아무것도 안함 (선택)
...
def save_original_file_to_blob(uploaded_file_obj, _container_client):
    ...
    try:
        ...
        # Blob 경로에 사용자 ID나 날짜 등을 추가하여 파일명 충돌 방지 및 관리 용이성 향상 가능
        original_blob_name = f"uploaded_originals/{datetime.now().strftime('%Y%m%d')}/{uploaded_file_obj.name}"
        blob_client_for_original = _container_client.get_blob_client(blob=original_blob_name)
        blob_client_for_original.upload_blob(uploaded_file_obj.getvalue(), overwrite=True) # 동일 파일명 덮어쓰기
        print(f"Original file '{uploaded_file_obj.name}' saved to Blob as '{original_blob_name}'.")
        return original_blob_name # 저장된 Blob 경로 반환 (URL 대신)
    except Exception as e:
...
if current_user_info.get("role") == "admin": # current_user_info는 로그인 후 설정됨
...
with chat_interface_tab:
    ...
    if "messages" not in st.session_state: # 세션별 메시지 저장소 초기화
        st.session_state["messages"] = []
        print("Chat messages list initialized for the current session.")
    ...
    for msg_item in st.session_state["messages"]:
        ...
        # HTML injection 방지를 위해 content 이스케이프 처리 고려
        # import html
    ...
    if st.button("📂 파일 첨부/숨기기", key="toggle_chat_uploader_final_button"):
        st.session_state.show_uploader = not st.session_state.get("show_uploader", False)

    chat_file_uploader_key = "chat_file_uploader_final_widget"
    ...
    with st.form("chat_input_form_final", clear_on_submit=True):
        ...
        with query_input_col:
            user_query_input = st.text_input("질문 입력:", placeholder="여기에 질문을 입력하세요...",
                                             key="user_query_text_input_final", label_visibility="collapsed")
...
if admin_settings_tab: # 관리자 탭 UI
    with admin_settings_tab:
        st.header("⚙️ 관리자 설정")
        st.subheader("👥 가입 승인 대기자")
        if not USERS: st.warning("사용자 정보를 로드할 수 없거나 비어있습니다 (USERS 딕셔너리).")
        else:
            ...
            if pending_approval_users:
                for pending_uid, pending_user_data in pending_approval_users.items():
                    with st.expander(f"{pending_user_data.get('name','N/A')} ({pending_uid}) - {pending_user_data.get('department','N/A')}"):
                        ...
                        if approve_col.button("승인", key=f"admin_approve_user_final_{pending_uid}"):
                        ...
                        if reject_col.button("거절", key=f"admin_reject_user_final_{pending_uid}"):
                            USERS.pop(pending_uid, None) # 키가 없을 경우 오류 방지
                            if save_data_to_blob(USERS, USERS_BLOB_NAME, container_client, "사용자 정보"):
        ...
        admin_file_uploader_key = "admin_file_uploader_final_widget"
        ...
        if container_client:
            ...
            if usage_data_from_blob and isinstance(usage_data_from_blob, list) and len(usage_data_from_blob) > 0 :
                ...
                # total_tokens가 없는 경우를 대비하여 0으로 처리
                if "total_tokens" not in df_usage_stats.columns:
                    df_usage_stats["total_tokens"] = 0
                    st.warning("'total_tokens' 컬럼이 일부 로그에 없어 0으로 간주합니다.")
        ...
        if container_client:
            try:
                blob_list_display = []
                # list_blobs()는 모든 blob을 가져오므로, 특정 경로 하위만 보려면 name_starts_with 사용 가능
                # 예: for blob_item in container_client.list_blobs(name_starts_with="app_data/", results_per_page=100):
