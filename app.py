...
try:
    ...
    if not cookie_secret_key:
        ...
        cookies = EncryptedCookieManager(
            prefix="gmp_chatbot_final_auth_v3/", # Prefix 변경하여 이전 쿠키와 충돌 방지
...
if "authenticated" not in st.session_state:
    ...
    if cookie_manager_ready: # 쿠키에서 인증 정보 복원 시도
            ...
            if (time.time() - login_time) < SESSION_TIMEOUT:
                ...
                try:
                    ...
                    if user_data_from_cookie:
                        ...
                        # 로그인 복원 시에는 messages를 초기화하지 않음 (사용자가 이어가고 싶을 수 있음)
                        # 대신, 명시적 로그인/로그아웃 시에만 messages를 초기화
                        print(f"User '{user_data_from_cookie.get('name')}' authenticated from cookie.")
                    else: # 쿠키 사용자 정보 비어있음
                        print("User data in cookie is empty. Clearing auth state for cookie.")
                        st.session_state["authenticated"] = False # 이중 확인
...
# 로그인 UI 전에 st.session_state.messages가 존재하지 않으면 초기화 (안전장치)
...
if not st.session_state.get("authenticated", False):
    ...
    with st.form("auth_form_final_v2", clear_on_submit=False): # form key 변경
        mode = st.radio("선택", ["로그인", "회원가입"], key="auth_mode_final_v2")
        uid = st.text_input("ID", key="auth_uid_final_v2")
        pwd = st.text_input("비밀번호", type="password", key="auth_pwd_final_v2")
        name, dept = "", ""
        if mode == "회원가입":
            name = st.text_input("이름", key="auth_name_final_v2")
            dept = st.text_input("부서", key="auth_dept_final_v2")
...
with top_cols_main[1]:
    ...
    if st.button("로그아웃", key="logout_button_final_v2"): # 버튼 키 변경
        st.session_state["authenticated"] = False
        st.session_state["user"] = {}
...
def save_original_file_to_blob(uploaded_file_obj, _container_client):
    ...
    try:
        ...
        original_blob_name = f"uploaded_originals/{datetime.now().strftime('%Y%m%d%H%M%S')}_{uploaded_file_obj.name}" # 파일명 중복 방지 강화
        blob_client_for_original = _container_client.get_blob_client(blob=original_blob_name)
        blob_client_for_original.upload_blob(uploaded_file_obj.getvalue(), overwrite=False) # 중복 파일명 시 덮어쓰지 않음 (선택)
...
with chat_interface_tab:
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = []
        print("Chat messages list initialized for the current session (chat tab).")
    ...
    if st.button("📂 파일 첨부/숨기기", key="toggle_chat_uploader_final_v2_button"):
        st.session_state.show_uploader = not st.session_state.get("show_uploader", False)

    chat_file_uploader_key = "chat_file_uploader_final_v2_widget"
    ...
    with st.form("chat_input_form_final_v2", clear_on_submit=True): # form key 변경
        ...
        with query_input_col:
            user_query_input = st.text_input("질문 입력:", placeholder="여기에 질문을 입력하세요...",
                                             key="user_query_text_input_final_v2", label_visibility="collapsed")
    ...
    if send_query_button and user_query_input.strip():
        if not openai_client:
            ...
            user_id_for_log = current_user_info.get("name", "anonymous_chat_user_runtime") # 수정된 변수명 사용
...
if admin_settings_tab: 
    with admin_settings_tab:
            ...
            if pending_approval_users:
                for pending_uid, pending_user_data in pending_approval_users.items():
                    with st.expander(f"{pending_user_data.get('name','N/A')} ({pending_uid}) - {pending_user_data.get('department','N/A')}"):
                        ...
                        if approve_col.button("승인", key=f"admin_approve_user_final_v2_{pending_uid}"):
                        ...
                        if reject_col.button("거절", key=f"admin_reject_user_final_v2_{pending_uid}"):
        ...
        admin_file_uploader_key = "admin_file_uploader_final_v2_widget"
